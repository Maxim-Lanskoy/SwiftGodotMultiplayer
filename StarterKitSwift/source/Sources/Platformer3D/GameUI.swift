...
class GameUI: CanvasLayer {
    ...
    override func _ready() {
        ...
        guard let node = getTree()?.root?.getNode(path: "Main/Player") else {
            GD.pushError("GameUI does not hold a reference to the Player node!")
            return
        }
        guard let player = node as? Player else {
            GD.pushError("GameUI is unable to cast the Player node!")
            return
        }
        player.coinCollected.connect { [weak self] total in
            guard let self = self else { return }
            self.onCoinCollected(total: total)
        }
    }
